 ...
 */
static struct dev_udp *dev_udp_create(uint16_t port,
	...
	struct dev_udp *du = calloc(1, sizeof(struct dev_udp));
	du->skt = skt;
	du->deliver = deliver;
	du->arg = arg;
	 ...
	 */
	earth.intr.register_dev(skt, read_avail, du);
 ...
 */
static void dev_udp_send(void *dst, char *buf, unsigned int size){
...
void dev_udp_setup(struct dev_udp_intf *dui){
	dui->create = dev_udp_create;
	dui->send = dev_udp_send;
}
