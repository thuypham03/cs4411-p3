 ...
 */
bool path_exec(char *exec, int argc, char **argv, bool background, gpid_t *ppid){
	char *argb;
	unsigned int total;
	...
	if (index(exec, '/') == 0) {
		...
		asprintf(&path, "%s.exe", exec);
	}
	bool r = spawn_fexec(path, argb, total, !background, 0, ppid);
	free(path);
 ...
 */
static void wait(gpid_t pid, bool interactive){
	struct proc *proc;
...
void parse(char *cmdline, int argc, char **argv, bool interactive){
