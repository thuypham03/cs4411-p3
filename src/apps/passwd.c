 ...
 */
bool pwdsvr_change(gpid_t svr, char *uname, char *opwd, char *npwd){
...
static void change_pwd(char *uname, char *opwd, char *npwd){
	bool r = pwdsvr_change(GRASS_ENV->servers[GPID_PWD], uname, opwd, npwd);
	if (r) {
...
int main(int argc, char **argv){
	...
	file_set_flags(GRASS_ENV->stdin.server, GRASS_ENV->stdin.file_no, FILE_ECHO);
	printf("login: "); fflush(stdout);
	if (fgets(user, MAX_LINE_SIZE, stdin) == 0) {
		return 1;
	}
	file_set_flags(GRASS_ENV->stdin.server, GRASS_ENV->stdin.file_no, 0);
